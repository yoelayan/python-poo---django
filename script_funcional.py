...
class DjangoEnviromentManager:
    ...
    def check_virtual_env(self):
        if os.environ.get("VIRTUAL_ENV"):
            ...
            self.print_message("Ejecuta 'source .venv/bin/activate' antes de ejecutar el script")
            return False
            # .\miEntorno1\Scripts\activate

    def check_postgresql(self):
        postgres_status = subprocess.getoutput(
    ...
    def manage_migrations(self, apply_migrations, create_migrations):
        ...
        if apply_migrations or create_migrations:
            self.print_message(
                "Comprobando si existen migraciones por aplicar..."
            )
        ...
        if pending_migrations:
            ...
            if create_migrations:
                ...
                self.print_message(
                    "No se crearán nuevas migraciones en este momento."
                )
            if apply_migrations:
                self.print_message(
                ...
                self.print_message(
                    "Las migraciones no se aplicarán en este momento."
                )
        else:
    ...
    def start_server(self, port):
        self.print_message(
            f"Iniciando el servidor Django en el puerto {port}..."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage Django Enviroment."
    )
    parser.add_argument(
        '-a', '--aply-migrations',
        action='store_true',
    ...
    if manager.virtual_env_active:
        manager.manage_migrations(
            args.apply_migrations,
            args.create_migrations
        )
